...
def write_event(client, memory_id, unique_actor_id, unique_session_id, unique_message):
    return create_event_with_fixed_retry(
        client,
        memory_id,
        unique_actor_id,
        unique_session_id,
...
for batch_num in range(0, total_records, batch_size):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(batch_num, batch_end):
            ...
            futures.append(
                executor.submit(write_event, client, short_term_id, unique_actor_id, unique_session_id, unique_message)
            )
    ...
    print(f"  第 {batch_num//batch_size + 1} 批完成，成功写入 {success_count} 条，失败 {failed_count} 条")
